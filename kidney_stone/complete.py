...
class Config:
    ...
    BASE_DIR = Path("/home/mailabs/Downloads/PYTHON/nnunet_data")  # Change if not on Kaggle
...
def find_dataset():
    ...
    possible_paths = [
        ...
        "./data/kssd2025",
        "../data/kssd2025",
        "/home/mailabs/Downloads/PYTHON/data",
        "/home/mailabs/Downloads/PYTHON/data/data",
    ]
...
def convert_to_nnunet_format():
    ...
    for pattern in ["**/Stone/*.jpg", "**/Stone/*.png", "**/*stone*.jpg", "**/*.tif", "**/*.tiff"]:
        found = list(config.DATA_DIR.glob(pattern))
        if found:
            # Filter out mask/label directories from images list if needed, though they might be in separate folders
            # For this specific dataset structure (image/ vs label/), we should be careful.
            # Let's filter to keep only those in 'image' folder if it exists in path
            found = [f for f in found if 'image' in str(f).lower() or 'stone' in str(f).lower()]
    ...
    for img_file in image_files:
        possible_masks = [
            ...
            # New dataset structure: .../data/image/xxx.tif -> .../data/label/xxx.tif
            img_file.parent.parent / "label" / img_file.name,
            img_file.parent.parent / "labels" / img_file.name,
            img_file.parent.parent / "mask" / img_file.name,
        ]
